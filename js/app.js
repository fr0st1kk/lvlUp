(() => {
    ...
    (() => {
        ...
        const priceNew = document.querySelector(".callback__price-new"), priceOld = document.querySelector(".callback__price-old");
        function getTimeRemaining(endtime) {
            let days, hours, minutes, seconds;
            const t = Date.parse(endtime) - Date.parse(new Date) + 18e5;
            if (t <= 0) {
                days = 0;
                hours = 0;
                minutes = 0;
                seconds = 0;
                priceOld.style.cssText = "color: rgb(26, 184, 26); text-decoration: none; font-size: 32px";
                priceNew.style.cssText = "display: none";
            } else days = Math.floor(t / (1e3 * 60 * 60 * 24)), seconds = Math.floor(t / 1e3 % 60), 
            minutes = Math.floor(t / 1e3 / 60 % 60), hours = Math.floor(t / (1e3 * 60 * 60) % 24);
            return {
                total: t,
                days,
                hours,
